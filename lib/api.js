...
class Api {
	...
	static Alerts(action) {
		return {
			...
			GetUnreadCount: ["GET", "odata/Alerts/UiPath.Server.Configuration.OData.GetUnreadCount(action)"],
	...
	static QueueItems(action) {
		return {
			...
			GetItemProcessingHistory: ["GET", "odata/QueueItems({id})/UiPathODataSvc.GetItemProcessingHistory(action)"],
	...
	static QueueProcessingRecords(action) {
		return {
			...
			RetrieveQueuesProcessingStatus: ["GET", "odata/QueueProcessingRecords/UiPathODataSvc.RetrieveQueuesProcessingStatus(action)"]
		}[action]
	}
	...
	static RobotLogs(action) {
		return {
			...
			Reports: ["GET", "odata/RobotLogs/UiPath.Server.Configuration.OData.Reports(action)"]
		}[action]
	}
	...
	static Robots(action) {
		return {
			...
			GetMachineName: ["GET", "odata/Robots/UiPath.Server.Configuration.OData.GetMachineNameToLicenseKeyMappings(action)"]
		}[action]
	}
	...
	static Users(action) {
		return {
			...
			GetCurrentPermissions: ["GET", "odata/Users/UiPath.Server.Configuration.OData.GetCurrentPermissions(action)"],
			GetCurrentUser: ["GET", "odata/Users/UiPath.Server.Configuration.OData.GetCurrentUser(action)"],
